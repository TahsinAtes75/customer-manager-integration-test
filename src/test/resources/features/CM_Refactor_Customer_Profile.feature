Feature: CM - Refactor Customer Profile

  Background:
    Given I set REST API url as "https://customer-manager.test.heymanai.com"
    And I set request header content type as JSON


  @refactorCustomerProfileStatus401
  Scenario Outline: Refactor Customer Profile - Access token validation
    And I set header "authorization" parameter with value "<tokenValue>"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | John       |
      | surname     | Doe        |
      |preferredName| Johny      |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 401
    And response body should contain value of "67005" for key "errors[0].code"
    And response body should contain value of "Access token is invalid" for key "errors[0].description"

    Examples:
      | tokenValue                                                                                                                                                                                                                                                                         |
      |                                                                                                                                                                                                                                                                                    |
      | asr                                                                                                                                                                                                                                                                                |
      | ayJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJIeW1uYWkiLCJzdWIiOiIwYTgxMThmNS1jYjljLTQ0NzAtOWUwMC0zMjAyOTA5OGE4MWIiLCJzY29wZSI6IkVNQUlMX1ZFUklGSUNBVElPTiIsImlhdCI6MTU4MDEzNDUxNywiZXhwIjoxNTgwMjIwOTE3fQ.azYVhfG_xvNlpwHoOqWJt_BKlhB4Euz0_s91SEqiB-kLiP1MyKUIMNn3KJR9zUJ_nZrs92Ot-MNBZpbNdMzj2A |

  @refactorCustomerProfileStatus400
  Scenario Outline: Refactor Customer Profile - Given names Validation Test
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI5YzFkNmMyNi1mOTQ3LTQ4MTUtYTM2NS1mZDM1NDM1OTYyODkiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiI3MWZkZmE3MC01NmIyLTRlNmItOTc4Mi03ODAyMjA4NzI2NWQiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTk2ODEyNzYxLCJleHAiOjE1OTc0MTc1NjF9.H7U-eyci4jIeMuMozUBw5llCxv7OTP1q-FeQc1yj3ADpZQFGlowM4i9e4GFecZw2J5ca7yJTMYZmYQDEuUhArA"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | <givenNamesValue> |
      | surname     | Doe               |
      |preferredName| Johny             |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 400
    And response body should contain value of "65000" for key "errors[0].code"
    And response body should contain value of "<fieldName>" for key "errors[0].field"
    And response body should contain value of "<message>" for key "errors[0].description"

    Examples:
      | givenNamesValue                                    | fieldName   | message                       |
      | fsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkha|  givenNames | size must be between 2 and 50 |
      | f                                                  |  givenNames | size must be between 2 and 50 |
      |                                                    |  givenNames | size must be between 2 and 50 |

  @refactorCustomerProfileStatus400
  Scenario Outline: Refactor Customer Profile- Surname Validation Test
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI5YzFkNmMyNi1mOTQ3LTQ4MTUtYTM2NS1mZDM1NDM1OTYyODkiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiI3MWZkZmE3MC01NmIyLTRlNmItOTc4Mi03ODAyMjA4NzI2NWQiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTk2ODEyNzYxLCJleHAiOjE1OTc0MTc1NjF9.H7U-eyci4jIeMuMozUBw5llCxv7OTP1q-FeQc1yj3ADpZQFGlowM4i9e4GFecZw2J5ca7yJTMYZmYQDEuUhArA"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | John          |
      | surname     | <surnameValue>|
      |preferredName| Johny      |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 400
    And response body should contain value of "65000" for key "errors[0].code"
    And response body should contain value of "<fieldName>" for key "errors[0].field"
    And response body should contain value of "<message>" for key "errors[0].description"

    Examples:
      | surnameValue                                       | fieldName   | message                       |
      | fsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkha| surname     | size must be between 2 and 50 |
      | f                                                  | surname     | size must be between 2 and 50 |
      |                                                    | surname     | size must be between 2 and 50 |
  @refactorCustomerProfileStatus400
  Scenario Outline: Refactor Customer Profile- Preferred Name Validation Test
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI5YzFkNmMyNi1mOTQ3LTQ4MTUtYTM2NS1mZDM1NDM1OTYyODkiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiI3MWZkZmE3MC01NmIyLTRlNmItOTc4Mi03ODAyMjA4NzI2NWQiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTk2ODEyNzYxLCJleHAiOjE1OTc0MTc1NjF9.H7U-eyci4jIeMuMozUBw5llCxv7OTP1q-FeQc1yj3ADpZQFGlowM4i9e4GFecZw2J5ca7yJTMYZmYQDEuUhArA"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | John          |
      | surname     | wilmorth|
      |preferredName| <preferredNameValue>     |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 400
    And response body should contain value of "65000" for key "errors[0].code"
    And response body should contain value of "<fieldName>" for key "errors[0].field"
    And response body should contain value of "<message>" for key "errors[0].description"

    Examples:
      | preferredNameValue                                  |fieldName         | message                       |
      | fsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkhfsfkhsjfkha| preferredName     | size must be between 2 and 50 |
      | f                                                  | preferredName    | size must be between 2 and 50 |
      |                                                    | preferredName    | size must be between 2 and 50 |
  @refactorCustomerProfileStatus400
  Scenario Outline: Refactor Customer Profile- date of Birth Validation Test
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI5YzFkNmMyNi1mOTQ3LTQ4MTUtYTM2NS1mZDM1NDM1OTYyODkiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiI3MWZkZmE3MC01NmIyLTRlNmItOTc4Mi03ODAyMjA4NzI2NWQiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTk2ODEyNzYxLCJleHAiOjE1OTc0MTc1NjF9.H7U-eyci4jIeMuMozUBw5llCxv7OTP1q-FeQc1yj3ADpZQFGlowM4i9e4GFecZw2J5ca7yJTMYZmYQDEuUhArA"
    And I set request body with information given in the following table
      |dateOfBirth  |<dateOfBirthValue> |
      | givenNames  | John          |
      | surname     | wilmorth|
      |preferredName| Jimmy    |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 400
    And response body should contain value of "65000" for key "errors[0].code"
    And response body should contain value of "<fieldName>" for key "errors[0].field"
    And response body should contain value of "<message>" for key "errors[0].description"

    Examples:
      | dateOfBirthValue                                   |fieldName        | message                       |
      | 2020-01-03                                         | dateOfBirth    | Age must be 18 years old or older |
      | 1881-09-07                                         | dateOfBirth    | Age must be 120 years old or younger |
      |                                                    | dateOfBirth    | Date can not be empty |
      |2020-10-10                                          | dateOfBirth    |Date can not be a future date|


  @refactorCustomerProfileStatus200
  Scenario: Refactor Customer Profile - Happy Path
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiIxZmZkYmJiMy00MTAwLTQ5MWUtOTQzYy1mOTAwNzM4NTU1M2QiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiI3NzZmZmMzMS0xOTZhLTQwMWUtODNmNS00NmQ0M2ZhNjQ5NTEiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTk2ODE0ODgzLCJleHAiOjE1OTc0MTk2ODN9.m8vohSZ6z41qoiKUj5q4KqjaJSopUeBzI58_yY9scIGFMl5ZST10jIKzR4QNVjweJzhdzw0BWKkgl4vaCJXXNQ"
    And I set request body with information given in the following table
      |dateOfBirth  |1976-06-05  |
      | givenNames  | jamess       |
      | surname     | Deenerr       |
      |preferredName| jammyee     |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 200
    And response body should be following json
    """
    {
  "email": "yik2006@yopmail.com",
  "givenNames": "jamess",
  "surname": "Deenerr",
  "preferredName": "jammyee",
  "dateOfBirth": "1976-06-05",
  "onboardingStatus": "NAME_DOB_CAPTURED",
  "customerStatus": "PROSPECT",
  "idStatus": "none",
  "photoStatus": "none"
}
    """

  @refactorCustomerProfileStatus401
  Scenario: Refactor Customer Profile - Expired Customer Access Token
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiJkNzk0ZTFmYS01NWIzLTRiZTItYjkwZS1iZWRkOGM1YzRlNGEiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiIzZmQ3MzNlMi0wNTNlLTRlYzUtYWIxNi1kZDQzNjVjOTY5MjkiLCJzY29wZSI6IkNVU1RPTUVSIiwiaWF0IjoxNTkxMzcwODMzLCJleHAiOjE1OTE5NzU2MzN9.o1ZxID33NXiYZiUgTixb4KNxUwPmlmfgwEcN_KRtdOhQO98XQP9QXLOlKE4m89TjJJs-yhqHw2HUU5rdFhGokQ"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | John       |
      | surname     | Doe        |
      |preferredName| Johny      |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 401
    And response body should contain value of "67005" for key "errors[0].code"
    And response body should contain value of "Access token is invalid" for key "errors[0].description"

  @refactorCustomerProfileStatus403
  Scenario: Refactor Customer Profile - Agent Access Token
    And I set header "authorization" parameter with value "eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI3OGUzOTllYS1lYjc0LTQyM2YtODBiZS0xMDgwMmYyZjBjZDkiLCJpc3MiOiJIeW1uYWkiLCJzdWIiOiJBZ2VudENhdCIsInNjb3BlIjoiQUdFTlQiLCJpYXQiOjE1OTY3MDg3ODUsImV4cCI6MTU5NzMxMzU4NX0.ETYmy-72DzUR7SUbThjoyW4b4AUp7Aof4jPFxXcUYB9udzzWiScKt3OneUp8y_7FzHjnGwqdfF67Sz6Jg4J4pA"
    And I set request body with information given in the following table
      |dateOfBirth  |1980-08-07  |
      | givenNames  | John       |
      | surname     | Doe        |
      |preferredName| Johny      |
    When I PUT request to "/v1/customers/profile"
    Then response status code should be 403
    And response body should contain value of "65013" for key "errors[0].code"
    And response body should contain value of "Access denied" for key "errors[0].description"
